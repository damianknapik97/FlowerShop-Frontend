...
export class OrderAdministrationComponent implements OnInit {
  ...
  @Input() public sortingProperties: string[] = [
    'NONE'
  ];
  public dataLoaded = false;
  public content: OrderDTO[];

  constructor(private service: OrderAdministrationService, private snackBar: MatSnackBar) { }

  ngOnInit() {
    this.retrieveSortingProperties();
    this.retrieveOrdersPage(this.page, this.pageSize, this.sortingProperties[0]);
  }

  public retrieveSortingProperties(): void {
    this.service.retriveSortingProperties().subscribe(
      (res: string[]) => {
  ...
  public retrieveOrdersPage(page: number, pageSize: number, sortingProperty: string): void {
    this.service.retrieveOrdersPage(page, pageSize, sortingProperty).subscribe(
      ...
      (err: any) => {
        this.snackBar.open('Couldn\t retrieve orders page', 'Error', {duration: 3000});
        console.log(err);
      }
    );
  }
